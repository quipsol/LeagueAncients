...
func _init() -> void:
	...
	# Pack C# script .uid files from ZoeModCode/ so their uid:// ids
	# resolve at runtime. The scripts live outside the asset folder, so the pass
	# above never sees their .uid files.
	print("Packing C# .uid files from: res://ZoeModCode")
	_pack_cs_uid_files_recursive(packer, "res://ZoeModCode")
